...
- [ ] **Step 1: データの型拡張 (Model)**
    - [ ] `Riddle.java` に `type` フィールド (text/click) を追加
- [ ] **Step 2: データ更新 (JSON)**
    - [ ] `riddles.json` の既存データに `"type": "text"` を追加
    - [ ] 新しいクリック問題 (ID:4) を追加 (`"type": "click"`)
- [ ] **Step 3: 画面の改修 (UI/UX)**
    - [ ] `style.css` にクリック可能な文字用のスタイルを追加
    - [ ] `quiz.html` に `th:if` で入力欄とクリックエリアを出し分ける処理を追加
    - [ ] JavaScriptでクリック時に正解を送信する仕組みを実装
    
## 🥅 ゴール5: 本番環境へのデプロイ
作成した機能を本番サーバー (Render) に反映し、スマホ実機で動作確認を行う。

- [ ] **Step 1: コードの反映**
    - [ ] 作業ブランチを `main` にマージしてプッシュ
- [ ] **Step 2: 動作確認**
    - [ ] Renderのデプロイ完了を待つ
    - [ ] スマホでURLにアクセスし、メニューやクリック問題が動くか確認
---
