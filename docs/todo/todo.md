...
- [ ] **Step 1: データの型拡張 (Model)**
    - [ ] `Riddle.java` に `imageUrl` フィールドを追加
    - [ ] コンストラクタ変更に伴うテスト修正 (Red)
- [ ] **Step 2: データ更新 (JSON & Image)**
    - [ ] `riddles.json` に Level 5 (type="image-map") を追加
    - [ ] 暗号画像ファイル (`level5.png`) を作成して配置
    - [ ] Repositoryテストの追加
- [ ] **Step 3: コントローラー拡張**
    - [ ] `MainController` に画像クリック判定用 (GET) のメソッドを追加
- [ ] **Step 4: 画面実装**
    - [ ] `quiz.html` にイメージマップ表示処理を追加
---
